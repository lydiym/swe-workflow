...
- swe-workflow skills list --agent <agent> [--project]
- swe-workflow skills create <name>
- swe-workflow skills info <name>
"""
...
from ..config import COLORS, Settings, console
from .load import list_skills
...
def _list(agent: str, *, project: bool = False) -> None:
    ...
    if project:
        ...
        if not project_skills_dir.exists() or not any(project_skills_dir.iterdir()):
            ...
            console.print(
                "\n[dim]Create a project skill:\n  swe-workflow skills create my-skill --project[/dim]",
                style=COLORS["dim"],
            )
            return
        ...
        if not skills:
            ...
            console.print(
                "[dim]Skills will be created in ~/.swe-workflow/agent/skills/ when you add them.[/dim]",
                style=COLORS["dim"],
            )
            console.print(
                "\n[dim]Create your first skill:\n  swe-workflow skills create my-skill[/dim]",
                style=COLORS["dim"],
            )
            return
...
# license: Apache-2.0
# compatibility: Designed for swe-workflow
# metadata:
#   author: your-org
#   version: "1.0"
