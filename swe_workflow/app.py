"""Textual UI application for swe-workflow."""
...
from .textual_adapter import TextualUIAdapter, execute_task_textual
from .widgets.approval import ApprovalMenu
...
class TUI(App):
    """Main Textual application for swe-workflow."""
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        """Initialize the SWE-Workflow application.
    ...
    async def _load_conversation_history(self) -> None:
        ...
        try:
            ...
            if hasattr(state, "values") and "messages" in state.values:
                ...
                # Initialize the UI adapter's current tool messages tracking if available
                if self._ui_adapter:
                ...
                for msg in all_messages:
                    if msg.type == "tool" or "tool" in type(msg).__name__.lower():
    ...
    async def _handle_command(self, command: str) -> None:
        ...
        if cmd in ("/quit", "/exit", "/q"):
            ...
            try:
                ...
                await self._mount_message(SystemMessage(f"swe-workflow version: {__version__}"))
            except Exception:
                await self._mount_message(SystemMessage("swe-workflow version: unknown"))
        elif cmd == "/clear":
