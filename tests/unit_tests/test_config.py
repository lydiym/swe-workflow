...
from swe_workflow.config import _find_project_agent_md, _find_project_root
...
class TestProjectAgentMdFinding:
    ...
    def test_find_agent_md_in_swe_workflow_dir(self, tmp_path: Path) -> None:
        """Test finding AGENTS.md in .swe_workflow/ directory."""
        project_root = tmp_path / "project"
        project_root.mkdir()

        # Create .swe_workflow/AGENTS.md
        swe_workflow_dir = project_root / ".swe_workflow"
        swe_workflow_dir.mkdir()
        agent_md = swe_workflow_dir / "AGENTS.md"
    ...
    def test_find_agent_md_in_root(self, tmp_path: Path) -> None:
        ...
        # Create root-level AGENTS.md (no .swe_workflow/)
    ...
    def test_both_agent_md_files_combined(self, tmp_path: Path) -> None:
        ...
        swe_workflow_dir = project_root / ".swe_workflow"
        swe_workflow_dir.mkdir()
        agents_md = swe_workflow_dir / "AGENTS.md"
        agents_md.write_text("In .swe_workflow/")

        root_md = project_root / "AGENTS.md"
        root_md.write_text("In root")

        # Should return both, with .swe_workflow/ first
        result = _find_project_agent_md(project_root)
        assert len(result) == 2
