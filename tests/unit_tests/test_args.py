...
from swe_workflow.main import parse_args
...
class TestInitialPromptArg:
    ...
    def test_short_flag(self) -> None:
        ...
        with patch.object(sys, "argv", ["swe_workflow", "-m", "hello world"]):
            args = parse_args()
        assert args.initial_prompt == "hello world"

    def test_long_flag(self) -> None:
        """Verify --message sets initial_prompt."""
        with patch.object(sys, "argv", ["swe_workflow", "--message", "hello world"]):
            args = parse_args()
        assert args.initial_prompt == "hello world"

    def test_no_flag(self) -> None:
        """Verify initial_prompt is None when not provided."""
        with patch.object(sys, "argv", ["swe_workflow"]):
            args = parse_args()
    ...
    def test_with_other_args(self) -> None:
        ...
        with patch.object(
            sys, "argv", ["swe_workflow", "--agent", "myagent", "-m", "do something"]
        ):
            args = parse_args()
    ...
    def test_empty_string(self) -> None:
        ...
        with patch.object(sys, "argv", ["swe_workflow", "-m", ""]):
            args = parse_args()
