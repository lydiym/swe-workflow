...
from swe_workflow._version import __version__
...
def test_cli_version_flag() -> None:
    ...
    result = subprocess.run(
        [sys.executable, "-m", "swe_workflow.main", "--version"],
        capture_output=True,
        text=True,
        check=False,
    )
    ...
    assert f"swe_workflow {__version__}" in result.stdout
...
def test_version_slash_command_message_format() -> None:
    ...
    expected_message = f"swe_workflow version: {__version__}"
    assert "swe_workflow version:" in expected_message
...
def test_help_mentions_version_flag() -> None:
    ...
    result = subprocess.run(
        [sys.executable, "-m", "swe_workflow.main", "help"],
        capture_output=True,
        text=True,
        check=False,
    )
